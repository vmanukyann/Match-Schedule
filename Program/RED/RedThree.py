...
# Function to transform match data for Red 3
def transform_matches_data(matches):
    ...
    for match in sorted_matches:
        ...
        # Process red alliance, extracting only Red 3 (third position)
        red_teams = alliances.get('red', {}).get('team_keys', [])
        if len(red_teams) >= 3:
            team_number = red_teams[2][3:] if red_teams[2].startswith("frc") else red_teams[2]
...
def save_matches_to_file(matches_data, directory, filename="RedThree.json"):
    try:
